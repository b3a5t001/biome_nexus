...
public class FertilizedSpreadableBlock extends SpreadableBlock implements BlockEntityProvider, FertilizedBlockAccess {
    private final Block baseBlock;
    protected static final IntProperty FERT_STAGE = FertilizedBlockUtil.FERT_STAGE;

    public FertilizedSpreadableBlock(Block baseBlock, Settings settings) {
        super(settings.ticksRandomly());
        this.baseBlock = baseBlock;
        this.setDefaultState(this.stateManager.getDefaultState().with(SNOWY, false).with(FERT_STAGE, 1));
    }
    ...
    @Override
    public MapCodec<? extends SpreadableBlock> getCodec() {
        @SuppressWarnings("unchecked")
        MapCodec<? extends SpreadableBlock> codec = (MapCodec<? extends SpreadableBlock>)(MapCodec<?>) CODEC;
        return codec;
    }
}
